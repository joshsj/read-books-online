...
describe("Validation", () => {
  ...
  const runs = ["insert", "update"] as const;

  runs.forEach((op) => {
    describe(op[0]!.toUpperCase() + op.slice(1), () => {
      it("Passes for valid entities", async () => {
        op === "update" && (await model.create(entity));

        const result = repository[op](entity);
      ...
      it("Triggers for invalid fields", async () => {
        ...
        const result = repository[op]({ ...entity, min3: "ab" });
